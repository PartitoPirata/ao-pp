...
DEFINE('MATHGUARD_COLOR', "#aa0000");
DEFINE('MATHGUARD_SIZE', 4);
DEFINE('MATHGUARD_LINEHEIGHT', 3);
DEFINE('MATHGUARD_QUESTION', "Risolvi la domanda di sicurezza:");
DEFINE('MATHGUARD_FIELD_ANSWER',md5("bqueryeventzAnswer".(date("d")*date("W")*date("W"))));	// change this immediatelly to something else. eg. myanswer123
DEFINE('MATHGUARD_FIELD_CODE',md5("bqueryeventzCode".(date("d")*date("W"))));		// change this immediatelly to something else. eg. mysecretcodefield
...
class MathGuard {
	...
	function produceOutput($prime) {
		...
		$output = MATHGUARD_QUESTION .'<div id="mathguard_question" style="font-family: monospace; font-size:' . MATHGUARD_SIZE . 'px; font-weight: bold; color: ' . MATHGUARD_COLOR . '; padding:0px; margin: 0px; line-height:' . MATHGUARD_LINEHEIGHT . 'px;">' . MathGuard :: renderExpression($a, $b) . '</div><input id="mathguard_answer" type="input" name="'.MATHGUARD_FIELD_ANSWER.'" size="2" maxlength="2"/><input type="hidden" name="'.MATHGUARD_FIELD_CODE.'" value="'.$code.'" />';
